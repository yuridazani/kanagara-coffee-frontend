...
const UploadDPPage = () => {
    ...
    if (!reservation) {
        return <div className="text-center p-10 text-red-500">{t('Reservasi dengan nomor')} {reservationNumber} {t('tidak ditemukan.')}</div>;
    }
            ...
            <main className="pt-24 min-h-screen">
                <section className="py-20 px-4">
                    <div className="container mx-auto max-w-2xl text-center">
                        <h1 className="font-serif font-black text-4xl md:text-6xl text-wood-brown">{t('Upload Bukti DP')}</h1>
                        <p className="mt-4 text-charcoal/80">{t('Untuk Reservasi:')} <span className="font-mono font-bold">{reservation.reservationNumber}</span></p>

                        {isSubmitted ? (
                            <div className="mt-8 bg-soft-white p-8 rounded-lg shadow-lg">
                                <CheckCircle size={60} className="text-leaf-green mx-auto mb-4" />
                                <h2 className="font-serif font-bold text-2xl text-wood-brown">{t('Bukti Terkirim!')}</h2>
                                <p className="mt-2 text-charcoal/80">{t('Terima kasih. Tim kami akan segera memverifikasi bukti pembayaran Anda.')}</p>
                            ...
                            <form onSubmit={handleSubmit} className="mt-8 bg-soft-white p-8 rounded-lg shadow-lg space-y-6">
                                <div className="text-left">
                                    <p>{t('Silakan lakukan transfer DP sebesar')} <span className="font-bold">{t('Rp 100.000')}</span> {t('ke rekening berikut:')}</p>
                                    <p className="mt-2 font-bold">{t('BCA: 123456789 a/n Kanagara Coffee')}</p>
                                </div>
                                <div>
                                    <label htmlFor="proof" className="font-bold text-charcoal">{t('Upload Bukti Transfer')}</label>
                                    <input 
                                        type="file" 
                                        id="proof" 
                                        required 
