...
import { dummyFeedback } from '../data/adminData'; // ✅ data dummy

const KelolaFeedbackPage = () => {
    const { t } = useTranslation();

    const [feedbackList, setFeedbackList] = useState(dummyFeedback); // ✅ gunakan data dummy
    const [loading, setLoading] = useState(false); // ✅ tidak perlu loading
    const [filterRating, setFilterRating] = useState(0);
    const [currentPage, setCurrentPage] = useState(1);
    const itemsPerPage = 5;

    // Hapus useEffect untuk fetch data ❌

    // Fungsi delete interaktif
    const deleteFeedback = (id) => {
        if (window.confirm(t('Yakin ingin menghapus feedback ini?'))) {
            setFeedbackList(prev => prev.filter(fb => fb.id !== id));
            toast.success(t('Feedback berhasil dihapus.'));
        }
    };
