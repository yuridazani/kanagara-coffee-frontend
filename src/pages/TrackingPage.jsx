...
const StatusCard = ({ status, reservationNumber, name, date, time, rescheduleHistory, t }) => {
    ...
    switch (status) {
        case 'Menunggu Konfirmasi':
            infoText = "Reservasi Anda telah kami terima dan sedang menunggu konfirmasi dari tim kami.";
            break;
        case 'Dikonfirmasi':
            infoText = "Reservasi Anda telah dikonfirmasi! Silakan lakukan pembayaran DP untuk mengamankan tempat Anda.";
            actionButton = <Link to={`/upload-dp/${reservationNumber}`} className="bg-wood-brown text-white font-bold py-2 px-4 rounded-full text-sm">{t('Upload Bukti DP')}</Link>;
            break;
        case 'DP Dibayar':
            infoText = "Terima kasih, DP Anda telah diverifikasi. Anda sekarang dapat memilih menu untuk acara Anda.";
            actionButton = <Link to={`/pilih-menu/${reservationNumber}`} className="bg-wood-brown text-white font-bold py-2 px-4 rounded-full text-sm">{t('Pilih Menu Sekarang')}</Link>;
            break;
        case 'Menu Dipilih':
            infoText = "Pilihan menu Anda telah kami catat. Sampai jumpa di Kanagara!";
            break;
        case 'Bukti DP Diupload':
            infoText = "Bukti DP Anda sedang diverifikasi oleh tim kami. Mohon tunggu konfirmasi lebih lanjut.";
            break;
        case 'Selesai':
            infoText = "Terima kasih atas kunjungan Anda! Kami tunggu kedatangannya kembali.";
            break;
        case 'Ditolak':
            infoText = "Maaf, reservasi Anda tidak dapat kami proses. Silakan hubungi kami untuk informasi lebih lanjut.";
            break;
        default:
            infoText = "Status tidak diketahui.";
    }
                        ...
                        {rescheduleHistory.map((history, index) => (
                            <div key={index} className="mb-3 last:mb-0">
                                <div className="flex items-center text-sm text-blue-700">
                                    ...
                                    <span className="font-medium">{t('Perubahan')} {index + 1}: {history.oldDate} {history.oldTime} → {history.newDate} {history.newTime}</span>
                                </div>
                                {history.reason && (
            ...
            <main className="pt-24 min-h-screen bg-cream">
                <section className="py-20 px-4">
                    <div className="container mx-auto max-w-2xl text-center">
                        ...
                        <p className="mt-4 text-charcoal/80">{t('Masukkan nomor reservasi dan nama Anda untuk melihat status terbaru.')}</p>
                        ...
                        {notFound && <p className="mt-6 text-red-600 font-bold">{t('Reservasi tidak ditemukan. Pastikan nomor reservasi dan nama Anda sesuai.')}</p>}
                        ...
                        <div className="mt-16 bg-leaf-green/10 border border-leaf-green/30 p-6 rounded-lg text-center">
                            <h3 className="font-bold text-wood-brown flex items-center justify-center gap-2"><Phone size={18}/>{t('Butuh Bantuan?')}</h3>
                            <p className="text-charcoal/80 text-sm mt-2">{t('Jika Anda memiliki pertanyaan atau kendala terkait reservasi, jangan ragu untuk menghubungi kami melalui WhatsApp di nomor berikut:')}</p>
                            <a href="https://wa.me/6281234567890" target="_blank" rel="noopener noreferrer" className="font-bold text-leaf-green mt-3 inline-block">{t('0812-3456-7890 (Admin Kanagara)')}</a>
                        </div>
                    </div>
                </section>
            </main>
