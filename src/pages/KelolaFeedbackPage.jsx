...
import { dummyFeedback } from '../data/adminData'; // <-- fallback dummy
...
const KelolaFeedbackPage = () => {
  ...
  // Ambil data dari backend, fallback ke dummy kalau gagal
  const fetchFeedback = async () => {
    try {
      ...
      toast.error(t('Gagal memuat data dari server, menampilkan data dummy.'));
        ...
        <div className="flex gap-2">
          <button
            ...
            onClick={async () => {
              ...
              const toastId = toast.loading(t('Menghapus...'));
              try {
                await axiosClient.delete(`/feedback/${id}`);
                toast.success(t('Feedback dihapus.'), { id: toastId });
                fetchFeedback();
              } catch (error) {
                // Jika gagal hapus di server, hapus di dummy
                setFeedbackList((prev) => prev.filter((fb) => fb.id !== id));
                toast.error(t('Gagal menghapus di server, dihapus dari dummy.'), { id: toastId });
              }
            }}
          >
            {t('Hapus')}
  ...
  return (
    <div className="bg-soft-white p-8 rounded-xl shadow-lg space-y-6">
      <div>
        <h2 className="font-serif font-black text-2xl text-wood-brown">
          {t('Ulasan & Feedback Pengunjung')}
        </h2>
        <p className="text-charcoal/70 mt-1">
          {t('Lihat, analisis, dan kelola ulasan yang masuk dari pengunjung.')}
        </p>
      </div>
        ...
        <div className="space-y-6">
          {paginatedFeedback.length > 0 ? (
                ...
                <div className="flex justify-between items-start border-b pb-3 mb-3">
                  <div>
                    ...
                    <p className="text-xs text-charcoal/60">
                      {t('Kunjungan:')} {fb.visitDate} @ {fb.visitTime}
                    </p>
                  </div>
                  <button
            ...
            <p className="text-center p-8 text-charcoal/60 bg-cream rounded-lg">
              {t('Belum ada feedback yang sesuai dengan filter.')}
            </p>
          )}
        </div>
        ...
        {filteredFeedback.length > 0 && (
          <div className="flex flex-col items-center mt-8 space-y-2">
            <div className="text-sm text-charcoal/70">
              {t('Menampilkan')} {Math.min((currentPage - 1) * itemsPerPage + 1, filteredFeedback.length)} - {Math.min(currentPage * itemsPerPage, filteredFeedback.length)} {t('dari')} {filteredFeedback.length} {t('feedback')}
            </div>
